...
class CQDINO(DINO):
    ...
    def _init_layers(self) -> None:
        ...
        self.language_loss_func = AsymmetricLoss(gamma_neg=7, gamma_pos=0, clip=0.05)
        ...
        self.all_node_info = self.language_model.all_node_info
    ...
    def create_parent_mask(self, image_tag):
        batch_size, num_classes = image_tag.shape
        mask = torch.ones_like(image_tag, dtype=torch.bool)

        for b in range(batch_size):
            positive_labels = torch.where(image_tag[b] > 0)[0]

            for label_ids in positive_labels:
                label_ids = label_ids.item()

                parents = self.all_node_info[str(label_ids)]['Ancestor Category IDs']

                if len(parents) != 0:
                    for parent in parents:
                        if image_tag[b, parent] == 0:
                            mask[b, parent] = False
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        parent_mask = self.create_parent_mask(image_categories)

        masked_logits = logits * parent_mask
        masked_image_categories = image_categories * parent_mask

        language_loss = self.language_loss_func(masked_logits, masked_image_categories)
    ...
    def predict(self, batch_inputs, batch_data_samples, rescale: bool = True):
        ...
        tag_output, tag_indexes, logits = self.language_model.generate(visual_features[-1], tagging_embed)  # tag words
